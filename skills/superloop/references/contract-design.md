...
For revision 2 and later, pass the exact prior proposal as `--parent <parent-proposal.yaml>` to validation, decision, and compilation. After validation, initialize a proposal-compiled contract only with `loopctl init <exact-metadata.name> --from loop.yaml --out .superloop/loops`; initialization must not rename the contract or relocate its reviewed persistence fields.
