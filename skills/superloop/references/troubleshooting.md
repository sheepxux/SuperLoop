...
```bash
loopctl doctor
loopctl status --root .superloop/loops --json
...
- **Missing current integrity binding**: stop the Runner, confirm there is no active lease, back up the loop directory, and run `loopctl migrate <loop-dir>`; never synthesize generation, contract, or strategy digests by hand. This command does not convert a Loop-Engineering v1 directory into SuperLoop v2.
